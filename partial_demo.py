...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        # torch tensors from ContactGen
        contacts_object, partition_logits, uv_object = sample_results   # logits [B, N, B_parts]

        # ====== for saving part maps (numpy) ======
        contacts_np   = contacts_object.squeeze(0).cpu().numpy()        # [N]
        logits_np     = partition_logits.squeeze(0).cpu().numpy()       # [N, B_parts]

        # softmax probs for saving (numpy)
        probs_np      = torch.softmax(
            torch.from_numpy(logits_np), dim=-1
        ).numpy()                                                       # [N, B_parts]

        hard_np       = logits_np.argmax(axis=-1)                       # [N], int labels
        obj_points_np = obj_verts[0].cpu().numpy()                      # [N, 3]

        save_dir = args.save_root
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, "sample_points.npy"), obj_points_np)
        np.save(os.path.join(save_dir, "contact_map.npy"),   contacts_np)
        np.save(os.path.join(save_dir, "part_logits.npy"),   logits_np)
        np.save(os.path.join(save_dir, "part_probs.npy"),    probs_np)
        np.save(os.path.join(save_dir, "part_hard.npy"),     hard_np)

        # ====== torch integer part IDs for optimization ======
        # shape: [B, N], dtype: Long (int64) → exactly what optimize_pose wants
        obj_partition = partition_logits.argmax(dim=-1)      # [B, N], LongTensor
        
        # Ensure contacts_object is [B, N] for optimize_pose
        if contacts_object.dim() == 3:
            contacts_object = contacts_object.squeeze(-1)  # [B, N, 1] -> [B, N]
        elif contacts_object.dim() == 1:
            contacts_object = contacts_object.unsqueeze(0)   # [N] -> [1, N]
        # keep contacts_object and uv_object as torch for optimize_pose

    # run grasp optimization (outside no_grad, because it uses autograd)
