...
def optimize_pose(model, mano_layer, obj_verts, obj_cmap, obj_partition, obj_uv,
    ...
    """
    Modified optimizer to handle per-part contact maps [B, N, 16]
    """
    model.eval()
    ...
    for it in range(global_iter):
        ...
        # obj_cmap is now [B, N, 16] - per-part contact maps
        # obj_partition is [B, N] - part IDs (0-15)
        # Gather the contact value for the correct part
        if obj_cmap.dim() == 3 and obj_cmap.shape[-1] == 16:
            # Per-part contact maps: gather the contact for each point's assigned part
            obj_cmap_gathered = torch.gather(obj_cmap, dim=2, index=obj_partition.unsqueeze(dim=-1)).squeeze(-1)  # [B, N]
        else:
            # Legacy format: [B, N, 1] or [B, N]
            obj_cmap_gathered = obj_cmap.squeeze(-1) if obj_cmap.dim() == 3 else obj_cmap  # [B, N]
        
        # pred_p_full is [B, N, 16] - per-part predictions
        pred_p = torch.gather(pred_p_full, dim=2, index=obj_partition.unsqueeze(dim=-1)).squeeze(-1)  # [B, N]
        loss_contact = w_contact * (torch.abs(pred_p) * obj_cmap_gathered).sum(dim=-1).mean(dim=0)
        loss += loss_contact
        ...
        uv_loss = w_uv * compute_uv_loss(uv_pred, obj_uv, weight=1.0+obj_cmap_gathered)
        loss += uv_loss
    ...
    for it in range(pose_iter):
        ...
        _, frames = mano_layer(torch.cat((global_pose, mano_pose), dim=1), th_betas=mano_shape, th_trans=mano_trans)
        uv_pred = compute_uv(frames, obj_verts, obj_partition)
        uv_loss = w_uv * compute_uv_loss(uv_pred, obj_uv, weight=1+obj_cmap_gathered)
        loss += uv_loss
        loss_info += "uv loss: {:.3f} | ".format(uv_loss.item())
