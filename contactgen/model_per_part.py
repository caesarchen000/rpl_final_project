...
class ContactGenModelPerPart(nn.Module):
    """
    Modified ContactGenModel that outputs per-part contact maps [B, N, 16]
    instead of a single combined contact map [B, N, 1]
    """
    def __init__(self, cfg, **kwargs):
        super(ContactGenModelPerPart, self).__init__()
        self.cfg = cfg
        ...
        # Modified: output 16 channels (one per hand part) instead of 1
        self.contact_decoder = Pointnet(in_dim=encode_dim + self.latentD, hidden_dim=self.hc, out_dim=self.num_parts)
    ...
    def encode(self, obj_cond, contacts_object, partition_object, uv_object):
        ...
        # For encoding, aggregate across parts (take max) to get [B, N, 1]
        if contacts_object.dim() == 3 and contacts_object.shape[-1] == self.num_parts:
            contacts_object_agg = contacts_object.max(dim=-1, keepdim=True)[0]  # [B, N, 1]
        else:
            contacts_object_agg = contacts_object  # Handle legacy shape [B, N, 1]
        _, contact_latent = self.contact_encoder(torch.cat([obj_cond, contacts_object_agg], -1))
    ...
    def decode(self, z_contact, z_part, z_uv, obj_cond, gt_partition_object=None):
        ...
        # Output per-part contact maps: [B, N, 16]
