...
from .model_per_part import ContactGenModelPerPart
from .hand_object_per_part import HandObjectPerPart
from .datasets.grab_dataset import Grab


class TrainerPerPart:
    ...
    def __init__(self, cfg, logger=None):
        ...
        self.model = ContactGenModelPerPart(cfg).to(self.device)
        self.hand_object = HandObjectPerPart(self.device)
        self.model.cfg = cfg
        ...
        if cfg.checkpoint is not None:
            ...
            # Check if this is a full checkpoint with epoch info (for resuming)
            if 'epoch' in checkpoint:
                self.start_epoch = checkpoint['epoch']
                self.logger('Resuming from epoch %d' % self.start_epoch)
                # Load optimizer state if available
                if 'optimizer' in checkpoint:
                    self.optimizer_net.load_state_dict(checkpoint['optimizer'])
                    self.logger('Loaded optimizer state')
                # Load scheduler state if available
                if 'scheduler' in checkpoint:
                    self.lr_scheduler.load_state_dict(checkpoint['scheduler'])
                    self.logger('Loaded scheduler state')
            # Load model weights
    ...
    def loss_net(self, dorig, drec):
        ...
        # Modified: Handle per-part contact maps [B, N, 16]
        target_contact = dorig['contacts_object'].to(device)  # [B, N, 16] - per-part contact maps
        pred_contact = drec['contacts_object']  # [B, N, 16] - predicted per-part contact maps
        
        # Compute weight based on max contact across all parts
        target_contact_max = target_contact.max(dim=-1)[0]  # [B, N] - max contact across parts
        weight = 1. + 5. * target_contact_max  # [B, N]
        
        # Compute per-part L1 loss
        contact_obj_sub = target_contact - pred_contact  # [B, N, 16]
        contact_obj_weighted = contact_obj_sub * weight.unsqueeze(-1)  # [B, N, 16]
        
        loss_contact_rec = F.l1_loss(contact_obj_weighted, 
                                    torch.zeros_like(contact_obj_weighted, device=target_contact.device, dtype=target_contact.dtype), 
                                    reduction='none')
    ...
    def fit(self, n_epochs=None, message=None):
        ...
        for epoch_num in range(self.start_epoch, n_epochs):
            ...
            if self.fit_net:
                with torch.no_grad():
                    eval_msg = TrainerPerPart.create_loss_message(eval_loss_dict_net, epoch_num=self.epoch_completed, it=len(self.ds_val))
