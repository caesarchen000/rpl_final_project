...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ContactGen Demo - Per-Part Contact Maps')
    ...
    sample_face_indices = sample[1]  # Store which faces were sampled
    sample_points = sample[0]  # Store sampled points for mapping back
    ...
    # Save contact maps and sampled points for visualization
    contacts_object_np = contacts_object.detach().cpu().numpy()  # (n_samples, 2048, 16) - per-part contact maps
    np.save(os.path.join(args.save_root, 'contact_maps.npy'), contacts_object_np)
    np.save(os.path.join(args.save_root, 'sample_points.npy'), sample_points)
    print(f"Saved contact maps: {contacts_object_np.shape} (n_samples={args.n_samples}, n_points=2048, n_parts=16)")
    print(f"  Each contact map has 16 channels, one for each hand part")
    ...
    for i in range(len(hand_verts)):
        ...
        # Save individual per-part contact map
        np.save(os.path.join(args.save_root, 'contact_map_{}.npy'.format(i)), contacts_object_np[i])  # (2048, 16)
